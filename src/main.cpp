...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for(unsigned int i = 1; i < 11; i++)
        {
            ...
            float angle = 20.0f * i;
            if ((i-1) % 3 == 0)
                angle = (float)glfwGetTime() * glm::radians(angle);
...
void processInput(GLFWwindow *window)
{
    ...
    const float cameraSpeed = 2.5f * deltaTime; // Delta time brings consistency in movement speed across different hardware performance
