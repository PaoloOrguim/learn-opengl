...
struct Material {
    ...
    glm::vec3 diffuse;
    glm::vec3 specular;
    float shininess; // Based on the materials table shininess needs to be 128
};

// Materials taken from the provided table (order preserved)
std::vector<Material> materials = {
    ...
    // black plastic
    { {0.0f, 0.0f, 0.0f}, {0.01f, 0.01f, 0.01f}, {0.5f, 0.5f, 0.5f}, 0.25f },
    // cyan plastic
    ...
    // white plastic
    { {0.0f, 0.0f, 0.0f}, {0.55f, 0.55f, 0.55f}, {0.7f, 0.7f, 0.7f}, 0.25f },
    // yellow plastic
    { {0.0f, 0.0f, 0.0f}, {0.5f, 0.5f, 0.0f}, {0.6f, 0.6f, 0.5f}, 0.25f },
    // black rubber
...
int main()
{
    ...
    unsigned int emissionMap = loadTexture("resources/textures/emission.png");
    ...
    int cols = (int)ceil(sqrt((float)n));
    int rows = (n + cols - 1) / cols;
    float spacing = 1.0f;        // distance between cubes
    float startX = 0.0f; //-((cols - 1) * spacing) / 2.0f;
    float startZ = 0.0f;        // base Z to put grid in front of camera
    float rowSpacingZ = 1.0f;    // spacing along Z between rows
